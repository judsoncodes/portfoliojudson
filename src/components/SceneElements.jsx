...
export function MarineLayer({ depth, foodRef, counts }) {
  ...
  const configs = {
    fg: { count: counts?.fg || 12, speed: 1.6, bounds: { x: 30, y: 20, z: 10 }, palette: ['#22d3ee', '#06b6d4'] },
    mid: { count: counts?.mid || 35, speed: 1.0, bounds: { x: 45, y: 30, z: 15 }, palette: ['#0891b2', '#0e7490'] },
    bg: { count: counts?.bg || 60, speed: 0.6, bounds: { x: 65, y: 40, z: 25 }, palette: ['#064e62', '#022c37'] }
  };
  ...
  return (
    <>
      <FishMesh 
        ...
        foodRef={foodRef} 
      />
      {/* Add secondary swarm for midground density - scaled for mobile */}
      {depth === 'mid' && (
        <FishMesh 
          count={Math.max(2, Math.floor(config.count * 0.5))} 
          speed={1.4} 
      ...
      {/* Add secondary swarm for background density - scaled for mobile */}
      {depth === 'bg' && (
        <FishMesh 
          count={Math.max(2, Math.floor(config.count * 0.6))} 
          speed={0.4} 
